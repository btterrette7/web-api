  ...
  if (!(options.omitPost === true)) {
    route.post(function (req, res, next) {
      runPrePromise(options.prePost, req, res, next).then(() => {
		model.create({"Id": req.body.Id, "Name": req.body.Name,"Email": req.body.Email,"Number": req.body.Number,"Notes": req.body.Notes, "Rating": req.body.Rating}).then((results) => {
    ...
    route.put(function (req, res, next) {
      runPrePromise(options.prePut, req, res, next).then(() =>{ 
		...
		uploadToS3(req.body.Id+"ResumeOverlay.png", req.body.ResumeOverlay).then((response) => {
			...
			model.update({"Id": req.body.Id, "Name": req.body.Name,"Email": req.body.Email,"Number": req.body.Number,"Notes": req.body.Notes, "Rating": req.body.Rating}, {
            where: {
              Id: req.params.Id
            }
          }).then((results) => {
